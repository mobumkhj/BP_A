...
- Dual-Track TDD 학습 및 진행했다.
- RED → GREEN → REFACTOR 순서를 지키며 진행했다.
- Report 및 Prompt 의 지속 작성을 통해 진행 내용 확인 및 중간 점검을 했다.
...
- 일부 FR에 대한 테스트는 작성하지 못 했다.
- src 폴더 이름이 다르게 생성되어 혼선 및 코드 재작업이 있었다.
...
- readme나 prompt, report 문서들 및 test_plan, defect_list 같은 문서들 업데이트 및 관리를 잘하자.
- commit 단위를 잘 쪼개기. 
